...
import pt.isel.ls.commandresults.getresult.GetCountActivitiesByRidResult;
...
public class GetCountActivitiesByRidHandler extends GetHandler implements CommandHandler {
    @Override
    public CommandResult execute(CommandRequest commandRequest) throws SQLException {
        ...
        try (Connection conn = commandRequest.getDataSource().getConnection()) {
            ...
            if (resultSet.next()) {
                ...
                String sql2 = "SELECT COUNT(*) FROM activities WHERE rid = ?";
                pstmt = conn.prepareStatement(sql2);
                pstmt.setInt(1, Integer.parseInt(stringRid));
                resultSet = pstmt.executeQuery();
                if (resultSet.next()) {
                    int numberOfActivities = resultSet.getInt("count");
                    return new GetCountActivitiesByRidResult(route, numberOfActivities, commandRequest.getHeaders());
                } else {
